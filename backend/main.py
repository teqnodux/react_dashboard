...
def get_deal_detail(deal_id: str):
    ...
    if _DATA_SOURCE == "mongodb":
        ...
        detail["docket_entries"] = [
            {
                ...
                "received_date": entry.received_date.isoformat() if entry.received_date else "",
                "title": entry.title,
                ...
                "relief_requested": entry.relief_requested,
                "legal_regulatory_significance": entry.legal_regulatory_significance,
                "proceeding_phase": entry.proceeding_phase,
        ...
        detail["docket_stakeholders"] = [
            {"name": sh.name, "role": sh.role, "filing_count": sh.filing_count,
             "position": sh.position, "opposition_type": sh.opposition_type,
             "status": sh.status, "intervenor_type": sh.intervenor_type}
            for sh in deal.docket_stakeholders
        ]
        detail["docket_conditions"] = [
            {"text": cond.text, "status": cond.status, "source": cond.source,
             "category": cond.category, "opposition_type": cond.opposition_type,
             "relief_type": cond.relief_type, "asked_in": cond.asked_in, "resolved_in": cond.resolved_in}
...
def get_tenk_analysis(deal_id: str):
    """Get all 10-K/10-Q analysis filings for a deal from MongoDB."""
    from config import MONGODB_URI, MONGODB_DB
    from pymongo import MongoClient
    from bson import ObjectId
    ...
    try:
        ...
        for doc in docs:
            ...
            filings.append({
                ...
                "filing_label": doc.get("form_type", ""),
                "ticker": deal_ticker,
...
def get_tenk_analysis_parsed(deal_id: str, record_id: str):
    """
    Download and parse 10-K/10-Q DOCX from MongoDB record.
    Returns structured data with hasComparison flag and both summary + fulsome when available.
    """
    import io
    import requests
    import tempfile
        ...
        for element in doc.element.body:
            if element.tag.endswith('p'):
                ...
                for p in doc.paragraphs:
                    if p._element == element and p.text:
                        lines.append(p.text)
                        break
            elif element.tag.endswith('tbl'):
                # Table - extract rows, join cells with 4+ spaces for parser compatibility
    ...
    try:
        ...
        response = {
            "hasComparison": has_comparison,
            "filing_type": filing_type,
            "filing_label": ten_k_ten_q.get("label") if ten_k_ten_q else filing_type,
            "ticker": deal_ticker,
            "company": deal_company,
            "generated": filing_date,
        ...
        if has_comparison:
            ...
            try:
                ...
                response["summary"] = {
                    "doc_type": exec_parsed.get("doc_type", "exec"),
                    "overview": exec_parsed.get("overview", ""),
                    "sections": exec_parsed.get("sections", []),
                }
            finally:
                os.unlink(tmp_path)
            ...
            try:
                ...
                response["fulsome"] = {
                    "doc_type": redline_parsed.get("doc_type", "redline"),
                    "comparison_header": redline_parsed.get("comparison_header", ""),
                    "redline_excerpts": redline_parsed.get("redline_excerpts", []),
                    "transition": redline_parsed.get("transition", ""),
                }
            finally:
                os.unlink(tmp_path)
            ...
            try:
                ...
                response["summary"] = {
                    "doc_type": overview_parsed.get("doc_type", "overview"),
                    "headline": overview_parsed.get("headline", ""),
                    "overview": overview_parsed.get("overview", ""),
                    "brief": overview_parsed.get("brief", ""),
                    "excerpts": overview_parsed.get("excerpts", []),
                    "sections": overview_parsed.get("sections", []),
                }
            finally:
                os.unlink(tmp_path)
...
@app.get("/api/deals/{deal_id}/docket-query/status")
def docket_query_status(deal_id: str):
    """Check if the query engine is available for a deal."""
    engine = _get_query_engine(deal_id)
    if not engine:
        return {"ready": False, "entry_count": 0}
    return {"ready": True, "entry_count": len(engine.enriched_entries)}


@app.post("/api/deals/{deal_id}/docket-query")
async def docket_query(deal_id: str, req: DocketQueryRequest):
