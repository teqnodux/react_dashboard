...
function renderProxyDetailContent(content: string): React.ReactNode {
  ...
  while (i < lines.length) {
    ...
    if (trimmed.startsWith('####')) {
      blocks.push(<h6 key={blocks.length} className="proxy-detail-h4">{trimmed.replace(/^#{4}\s*/, '')}</h6>);
      i++; continue;
    }
    if (trimmed.startsWith('###')) {
      blocks.push(<h6 key={blocks.length} className="proxy-detail-h3">{trimmed.replace(/^#{3}\s*/, '')}</h6>);
      i++; continue;
    }
    ...
    if (/^\d+\.\s/.test(trimmed)) {
      ...
      blocks.push(
        <ol key={blocks.length} className="proxy-detail-list" start={parseInt(items[0]?.num || '1')}>
          {items.map((item, ii) => <li key={ii} value={parseInt(item.num)}>{formatInline(item.text)}</li>)}
        </ol>
      );
      continue;
    }
    ...
    // Regular paragraph
    blocks.push(<p key={blocks.length} className="proxy-detail-para">{formatInline(trimmed)}</p>);
    i++;
  }
...
export default function DealDetail() {
  ...
  const [dmaViewMode, setDmaViewMode] = useState<'concise' | 'fulsome'>('concise');
  ...
  const [secDetailView, setSecDetailView] = useState<'summary' | 'full'>('summary');
  const [secDefaultView, setSecDefaultView] = useState<'summary' | 'full'>('summary');
  ...
  const [secFilingRole, setSecFilingRole] = useState<'target' | 'acquirer'>('target');
  ...
  const [tenkViewMode, setTenkViewMode] = useState<'summary' | 'detail'>('summary');
  ...
  const [globalSettings, setGlobalSettings] = useState<{ long_spread_bps: number; short_spread_bps: number }>({ long_spread_bps: 50, short_spread_bps: 20 });
  ...
  const [dmaStatus, setDmaStatus] = useState<'idle' | 'checking' | 'ready' | 'generating' | 'error'>('idle');
  ...
  const [uploadDocType, setUploadDocType] = useState<'dma_summary' | 'press_release' | 'proxy' | 'tenk'>('dma_summary');
  ...
  const [covenantStatus, setCovenantStatus] = useState<'idle' | 'checking' | 'ready' | 'generating' | 'error'>('idle');
  const [covenantError, setCovenantError] = useState<string | null>(null);
  const [terminationStatus, setTerminationStatus] = useState<'idle' | 'checking' | 'ready' | 'generating' | 'error'>('idle');
  ...
  const [docViewSource, setDocViewSource] = useState<{type: string; filename?: string} | null>(null);
  ...
  const [covenantPipelineStatus, setCovenantPipelineStatus] = useState<string>('idle');
  const [covenantPipelineStep, setCovenantPipelineStep] = useState<string>('');
  const [maeStatus, setMaeStatus] = useState<'idle' | 'checking' | 'ready' | 'running' | 'error'>('idle');
  ...
  const [terminationPipelineStatus, setTerminationPipelineStatus] = useState<string>('idle');
  const [terminationPipelineStep, setTerminationPipelineStep] = useState<string>('');
  ...
  const [pipelineRunFeedback, setPipelineRunFeedback] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/api/deals/${dealId}`)
      ...
      .then(data => {
        ...
        const sectionsToDisplay = data.concise_sections && data.fulsome_sections
          ? (dmaViewMode === 'concise' ? data.concise_sections : data.fulsome_sections)
          : data.dma_sections || [];
  ...
  useEffect(() => {
    ...
    const roleFilings = deal.ai_sec_filings.filter((f: any) => f._role === secFilingRole);
    if (roleFilings.length > 0 && !selectedSECFiling) {
      const sorted = [...roleFilings].sort((a: any, b: any) => new Date(b.date).getTime() - new Date(a.date).getTime());
    ...
    const resize = () => {
      ...
      try {
        ...
        if (doc) {
          ...
          const h = Math.max(doc.documentElement.scrollHeight, doc.body.scrollHeight, 800);
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/api/deals/${dealId}/dma-summary`)
      ...
      .then(data => {
        ...
        const sections = dmaViewMode === 'concise' ? data.concise_sections : data.fulsome_sections;
        (sections || []).forEach((_: any, idx: number) => ids.add(`section-${idx}`));
  ...
  const [unaffectedYf, setUnaffectedYf] = useState<{price: number; date: string} | null>(null);
  useEffect(() => {
    if (!dealId) return;
  ...
  function _buildTenkPeriods(filings: any[]) {
    const groups: Record<string, { key: string; label: string; filingType: string; overview: any; exec: any; redline: any; hasComparison: boolean; sortDate: string }> = {};
    for (const f of filings) {
      ...
      if (f.doc_type === 'overview' || f.doc_type === 'l123') {
        ...
        periodKey = parts.length === 2 ? parts[1].trim() : (f.filing_type || f.filename);
      }
      if (!groups[periodKey]) {
        groups[periodKey] = { key: periodKey, label: periodKey, filingType: f.filing_type, overview: null, exec: null, redline: null, hasComparison: false, sortDate: f.generated || f.filename };
      }
      if (f.doc_type === 'overview' || f.doc_type === 'l123') groups[periodKey].overview = f;
      else if (f.doc_type === 'exec' && f.transition) { groups[periodKey].exec = f; groups[periodKey].hasComparison = true; }
      else if (f.doc_type === 'exec' && !f.transition) { groups[periodKey].overview = f; }  // standalone exec = treat as overview
      else if (f.doc_type === 'redline') { groups[periodKey].redline = f; groups[periodKey].hasComparison = true; }
    }
  ...
  const selectedTenkGroup = tenkPeriods.find(p => p.key === selectedTenkPeriod) || null;
      ...
      if (secBatchMode) {
        ...
        const poll = setInterval(async () => {
          try {
            ...
            setSecProcessSuccess(`Processing ${status.completed}/${status.total} filings...${errText}`);
            if (status.done) {
              clearInterval(poll);
              setSecProcessSuccess(`Done: ${status.completed - errors}/${status.total} processed${errText}`);
          ...
          } catch { clearInterval(poll); setSecProcessing(false); }
        }, 3000);
      } else {
        ...
        const headline = data.result?.L1_headline || data.result?.filing_type || 'Filing';
  ...
  const handleViewSourceText = (docType: string, filename?: string) => {
    ...
    fetch(url)
      ...
      .catch(() => { setDocSourceText(null); setDocSourceLoading(false); });
  };
  ...
  const saveExpectedClose = async () => {
    ...
    try {
      ...
      setDealOverrides(prev => ({ ...prev, expected_close_date: closeInput }));
  ...
  const resetExpectedClose = async () => {
    ...
    try {
      ...
      setDealOverrides(prev => { const n = { ...prev }; delete n.expected_close_date; return n; });
      setEditingClose(false);
    } catch {}
  };
  ...
  const handleUnifiedUpload = async () => {
    ...
    try {
      if (uploadDocType === 'dma_summary') {
        ...
        if (!tlRes.ok) throw new Error('Timeline extraction failed: ' + await tlRes.text());
  ...
  const expandAllClauses = () => {
    const sectionsToDisplay = deal?.concise_sections && deal?.fulsome_sections
  ...
  const premium = deal.unaffected_price ?
    ((deal.offer_price - deal.unaffected_price) / deal.unaffected_price * 100) : 0;
  ...
  const stockComponentValue = isStockDeal && acquirerPrice ? deal.stock_ratio * acquirerPrice : 0;
  const liveOfferValue = isStockDeal && acquirerPrice
    ? deal.cash_per_share + stockComponentValue + deal.cvr_per_share + deal.special_div
    : deal.offer_price;
  const liveGrossSpread = liveOfferValue - deal.current_price;
  const liveGrossPct = deal.current_price ? (liveGrossSpread / deal.current_price) * 100 : 0;
  ...
  const computedCloseDate: string | null = (() => {
    ...
    if (/1H/.test(t) || /FIRST HALF/.test(t))     { start = new Date(+year, 0, 1);  end = new Date(+year, 5, 30); }
    else if (/2H/.test(t) || /SECOND HALF/.test(t)){ start = new Date(+year, 6, 1);  end = new Date(+year, 11, 31); }
    else if (/Q1/.test(t)) { start = new Date(+year, 0, 1);  end = new Date(+year, 2, 31); }
    else if (/Q2/.test(t)) { start = new Date(+year, 3, 1);  end = new Date(+year, 5, 30); }
    else if (/Q3/.test(t)) { start = new Date(+year, 6, 1);  end = new Date(+year, 8, 30); }
    else if (/Q4/.test(t)) { start = new Date(+year, 9, 1);  end = new Date(+year, 11, 31); }
    else if (/MID/.test(t)){ start = new Date(+year, 3, 1);  end = new Date(+year, 7, 31); }
  ...
  const effectiveCloseDate = dealOverrides.expected_close_date || computedCloseDate || deal.expected_close;
  ...
  const betaMultiplier = (spyCurrent && spyAtAnnounce) ? spyCurrent / spyAtAnnounce : 1;
  const breakPrice = deal.unaffected_price ? deal.unaffected_price * betaMultiplier : 0;
  const totalSpread = liveOfferValue - breakPrice;
  const ipc = totalSpread > 0 ? Math.min(100, Math.max(0, ((deal.current_price - breakPrice) / totalSpread) * 100)) : 0;
  const downsidePct = (breakPrice && deal.current_price) ? ((breakPrice - deal.current_price) / deal.current_price) * 100 : 0;

  // ── Cost of Funds: deal-type-aware ──
  const useLegacyBorrow = dealOverrides.borrow_rate_annual != null;
  const sofr = sofrRate ?? 0.0430;
  const longSpreadBps = dealOverrides.long_spread_bps ?? globalSettings.long_spread_bps;
  const shortSpreadBps = dealOverrides.short_spread_bps ?? globalSettings.short_spread_bps;
  ...
  const liveBorrowCost = useLegacyBorrow
    ? deal.current_price * dealOverrides.borrow_rate_annual * (liveDaysToClose / 365)
    : longCost - shortRebate;
  const liveNetSpread = liveGrossSpread - liveBorrowCost + deal.dividend_expected;
  const liveNetPct = deal.current_price ? (liveNetSpread / deal.current_price) * 100 : 0;
  const liveAnnualizedGross = liveDaysToClose ? (liveGrossPct / liveDaysToClose) * 365 : 0;
  const liveAnnualizedNet = liveDaysToClose ? (liveNetPct / liveDaysToClose) * 365 : 0;
  ...
  const liveDealValueBn = prData?.deal_value_bn
  ...
  return (
    <div className="dashboard">
      ...
      <header className="header-compact">
        <div className="header-left">
          ...
          <div className="deal-meta-row">
            ...
            <span>{liveDealValueBn ? `$${liveDealValueBn.toFixed(1)}B` : '—'}</span>
            <span className="meta-divider">•</span>
            <span>Close: {formatDate(effectiveCloseDate)}{dealOverrides.expected_close_date ? '' : ' (est)'}</span>
            <span className="meta-divider">•</span>
        ...
        <div className="header-metrics">
          ...
          <div className="hm highlight">
            ...
            <span className="hm-value">${liveGrossSpread.toFixed(2)} ({liveGrossPct.toFixed(1)}%)</span>
          </div>
          <div className="hm">
      ...
      <div className="tabs-container">
        ...
        {activeTab === 'tearsheet' && (
          <div className={`tearsheet-container ${isFullScreen ? 'fullscreen' : ''}`}>
            <div className="tearsheet-header">
              <h2>Deal Tearsheet</h2>
              <p className="muted">Comprehensive view of all deal metrics and data points</p>
            </div>
            ...
            <div className="tearsheet-table-wrapper">
              <table className="tearsheet-table">
                <thead>
                  <tr>
                    {/* Target Information */}
                    <th className="column-group-header" colSpan={expandedColumnGroups.has('target') ? 3 : 1}>
                    ...
                    <th className="column-group-header" colSpan={expandedColumnGroups.has('acquirer') ? 3 : 1}>
                    ...
                    {/* Timing */}
                    <th className="column-group-header" colSpan={expandedColumnGroups.has('timing') ? 5 : 3}>
                    ...
                    {/* Downsides */}
                    <th className="column-group-header" colSpan={expandedColumnGroups.has('downsides') ? 4 : 3}>
                ...
                <tbody>
                  <tr className="tearsheet-row">
                    ...
                    <td className="company-name">
                      ...
                      <div className="cell-secondary">{deal.target_ticker}</div>
                    </td>
                    {expandedColumnGroups.has('target') && (
                      <>
                        <td className="number-cell">
                          <span className={liveQuotes?.target_quote?.shares_outstanding ? "number-value" : "muted"}>
                            {liveQuotes?.target_quote?.shares_outstanding
                              ? (liveQuotes.target_quote.shares_outstanding / 1e6).toFixed(1)
                              : '—'}
                          </span>
                        </td>
                        <td className="value-cell">
                          <span className={liveQuotes?.target_quote?.market_cap ? "value-text" : "muted"}>
                    ...
                    <td className="company-name">
                      ...
                      <div className="cell-secondary">{deal.acquirer_ticker || 'N/A'}</div>
                    </td>
                    {expandedColumnGroups.has('acquirer') && (
                      <>
                        <td className="number-cell">
                          <span className={liveQuotes?.acquirer_quote?.shares_outstanding ? "number-value" : "muted"}>
                            {liveQuotes?.acquirer_quote?.shares_outstanding
                              ? (liveQuotes.acquirer_quote.shares_outstanding / 1e6).toFixed(1)
                              : '—'}
                          </span>
                        </td>
                        <td className="value-cell">
                          <span className={liveQuotes?.acquirer_quote?.market_cap ? "value-text" : "muted"}>
                    ...
                    <td className="price-cell">
                      <span className="price-value">
                        {deal.cash_per_share > 0 ? `$${deal.cash_per_share.toFixed(2)}` : deal.stock_ratio === 0 ? `$${deal.offer_price.toFixed(2)}` : '—'}
                      </span>
                    </td>
                    <td className="price-cell">
                      <span className="price-value">
                    ...
                    {expandedColumnGroups.has('consideration') && (
                      <>
                        <td className="price-cell">
                              ...
                              <div>
                                <div className="tooltip-section">
                                  <div className="tooltip-row">
                                    <span className="tooltip-label">Cash / Share</span>
                                    <span className="tooltip-value">
                                      {deal.cash_per_share > 0 ? `$${deal.cash_per_share.toFixed(2)}` : deal.stock_ratio === 0 ? `$${deal.offer_price.toFixed(2)}` : '—'}
                                    </span>
                                  </div>
                                  <div className="tooltip-row">
                                    <span className="tooltip-label">Stock Ratio</span>
                                    <span className="tooltip-value">
                                      {deal.stock_ratio > 0 ? `${deal.stock_ratio} × ${deal.acquirer_ticker}` : '—'}
                                    </span>
                                  </div>
                                  {deal.stock_ratio > 0 && acquirerPrice > 0 && (
                                    <div className="tooltip-row">
                                      <span className="tooltip-label">Stock Value</span>
                                      <span className="tooltip-value">
                                  ...
                                  <div className="tooltip-row">
                                    ...
                                    <span className="tooltip-value">
                                      {deal.cvr_per_share > 0 ? `$${deal.cvr_per_share.toFixed(2)}` : '—'}
                                    </span>
                                  </div>
                                  <div className="tooltip-row">
                                    <span className="tooltip-label">Special Dividend</span>
                                    <span className="tooltip-value">
                                      {deal.special_div > 0 ? `$${deal.special_div.toFixed(2)}` : '—'}
                                    </span>
                                  </div>
                                  <div className="tooltip-row">
                                    <span className="tooltip-label">Proration</span>
                                    <span className="tooltip-value">—</span>
                                  </div>
                                  <div className="tooltip-row">
                                    <span className="tooltip-label">Collar</span>
                                    <span className="tooltip-value">—</span>
                                  </div>
                                </div>
                                <div className="tooltip-section">
                                  <div className="tooltip-section-title">Totals</div>
                                  <div className="tooltip-row">
                                    <span className="tooltip-label">Per Share Value</span>
                                    <span className="tooltip-value">${liveOfferValue.toFixed(2)}</span>
                                  </div>
                                  <div className="tooltip-row">
                                    <span className="tooltip-label">Deal Value</span>
                                    <span className="tooltip-value">
                          ...
                          >
                            <span className="price-value">${liveOfferValue.toFixed(2)}</span>
                          </TearsheetTooltip>
                        </td>
                        <td className="percent-cell">
                          <span className="percent-value">
                            {deal.unaffected_price > 0 ? `${((liveOfferValue - deal.unaffected_price) / deal.unaffected_price * 100).toFixed(1)}%` : '—'}
                          </span>
                        </td>
                        <td className="value-cell">
                    ...
                    <td className="price-cell">
                      <TearsheetTooltip
                        ...
                        content={
                          <div>
                            <div className="tooltip-section">
                              <div className="tooltip-section-title">Target</div>
                              <div className="tooltip-row">
                                <span className="tooltip-label">Target Bid</span>
                                <span className="tooltip-value">
                                  {liveQuotes?.target_quote?.bid ? `$${liveQuotes.target_quote.bid.toFixed(2)}` : '—'}
                                </span>
                              </div>
                              <div className="tooltip-row">
                                <span className="tooltip-label">Target Last</span>
                                <span className="tooltip-value">
                                  {liveQuotes?.target_quote?.current_price ? `$${liveQuotes.target_quote.current_price.toFixed(2)}` : `$${deal.current_price.toFixed(2)}`}
                                </span>
                              </div>
                              <div className="tooltip-row">
                                <span className="tooltip-label">Target Ask</span>
                                <span className="tooltip-value">
                                  {liveQuotes?.target_quote?.ask ? `$${liveQuotes.target_quote.ask.toFixed(2)}` : '—'}
                                </span>
                              </div>
                            </div>
                            <div className="tooltip-section">
                              <div className="tooltip-section-title">Spread</div>
                              <div className="tooltip-row">
                                <span className="tooltip-label">Spread Bid</span>
                                <span className="tooltip-value">
                              ...
                              <div className="tooltip-row">
                                <span className="tooltip-label">Spread Last</span>
                                <span className="tooltip-value">
                                  ${liveGrossSpread.toFixed(2)}
                                </span>
                              </div>
                              <div className="tooltip-row">
                                <span className="tooltip-label">Spread Ask</span>
                                <span className="tooltip-value">
                            ...
                            <div className="tooltip-section">
                              <div className="tooltip-section-title">Volume</div>
                              <div className="tooltip-row">
                                <span className="tooltip-label">Vol (mm)</span>
                                <span className="tooltip-value">
                                  {liveQuotes?.target_quote?.volume && liveQuotes?.target_quote?.current_price
                                    ? `$${((liveQuotes.target_quote.volume * liveQuotes.target_quote.current_price) / 1000000).toFixed(2)}`
                                    : '—'}
                                </span>
                              </div>
                              <div className="tooltip-row">
                                <span className="tooltip-label">Vol (Shares)</span>
                                <span className="tooltip-value">
                                  {liveQuotes?.target_quote?.volume ? liveQuotes.target_quote.volume.toLocaleString() : '—'}
                                </span>
                              </div>
                              <div className="tooltip-row">
                                <span className="tooltip-label">30 Day Volume</span>
                                <span className="tooltip-value">—</span>
                              </div>
                            </div>
                          </div>
                      ...
                      >
                        <span className="price-value">
                          ${(liveQuotes?.target_quote?.current_price || deal.current_price).toFixed(2)}
                        </span>
                      </TearsheetTooltip>
                    </td>
                    <td className="price-cell">
                      <span className={liveQuotes?.acquirer_quote?.current_price ? "price-value" : "muted"}>
                    ...
                    {expandedColumnGroups.has('trading') && (
                      <>
                        <td className="price-cell">
                          <span className={liveQuotes?.target_quote?.bid ? "price-value" : "muted"}>
                            {liveQuotes?.target_quote?.bid
                        ...
                        <td className="price-cell">
                          <span className={liveQuotes?.target_quote?.ask ? "price-value" : "muted"}>
                            {liveQuotes?.target_quote?.ask
                        ...
                        <td className="volume-cell">
                          <span className={liveQuotes?.target_quote?.volume ? "volume-value" : "muted"}>
                    ...
                    <td className="spread-cell">
                      <TearsheetTooltip
                        ...
                        content={
                          <div>
                            <div className="tooltip-section">
                              <div className="tooltip-section-title">Calculation</div>
                              <div className="tooltip-row">
                                <span className="tooltip-label">Offer Value</span>
                                <span className="tooltip-value">${liveOfferValue.toFixed(2)}</span>
                              </div>
                              <div className="tooltip-row">
                                <span className="tooltip-label">Current Price</span>
                                <span className="tooltip-value">${deal.current_price.toFixed(2)}</span>
                              </div>
                              <div className="tooltip-row">
                                <span className="tooltip-label">Gross Spread $</span>
                            ...
                            <div className="tooltip-section">
                              <div className="tooltip-section-title">Returns</div>
                              <div className="tooltip-row">
                                <span className="tooltip-label">Days to Close</span>
                                <span className="tooltip-value">{liveDaysToClose}</span>
                              </div>
                              <div className="tooltip-row">
                                <span className="tooltip-label">Annualized Gross</span>
                                <span className="tooltip-value tooltip-badge positive">
                      ...
                      >
                        <span className="spread-dollars">${liveGrossSpread.toFixed(2)}</span>
                      </TearsheetTooltip>
                    </td>
                    <td className="spread-cell">
                      <span className="spread-pct">{liveGrossPct.toFixed(2)}%</span>
                    </td>
                    {expandedColumnGroups.has('gross') && (
                      <>
                        <td className="spread-cell">
                          <span className="spread-pct">{liveAnnualizedGross.toFixed(2)}%</span>
                        </td>
                      </>
                    )}
                        ...
                        content={
                          <div>
                            <div className="tooltip-section">
                              ...
                              <div className="tooltip-row">
                                <span className="tooltip-label">Gross Spread</span>
                                <span className="tooltip-value">${liveGrossSpread.toFixed(2)}</span>
                              </div>
                              <div className="tooltip-row">
                                <span className="tooltip-label">Borrow Cost</span>
                                <span className="tooltip-value tooltip-badge negative">
                                  -${liveBorrowCost.toFixed(2)}
                                </span>
                              </div>
                              <div className="tooltip-row">
                                <span className="tooltip-label">Expected Dividend</span>
                                <span className="tooltip-value tooltip-badge positive">
                                  +${deal.dividend_expected.toFixed(2)}
                                </span>
                              </div>
                              <div className="tooltip-row">
                                <span className="tooltip-label">Net Spread $</span>
                                <span className="tooltip-value tooltip-badge positive">
                            ...
                            <div className="tooltip-section">
                              ...
                              <div className="tooltip-row">
                                <span className="tooltip-label">Annualized Net</span>
                                <span className="tooltip-value tooltip-badge positive">
                            ...
                            <div className="tooltip-section">
                              <div className="tooltip-section-title">Cost of Funds</div>
                              <div className="tooltip-text">
                                SOFR: {(sofr * 100).toFixed(2)}%{sofrDate && ` (${sofrDate})`}
                              </div>
                              <div className="tooltip-text">
                                Long: SOFR + {longSpreadBps}bps = {(longRate * 100).toFixed(2)}% on ${longValue.toFixed(2)}
                              </div>
                              {isStockDeal && (
                                <div className="tooltip-text">
                                  Short rebate: SOFR - {shortSpreadBps}bps = {(shortRebateRate * 100).toFixed(2)}% on ${shortValue.toFixed(2)}
                                </div>
                              )}
                              <div className="tooltip-text">
                                Net financing: ${liveBorrowCost.toFixed(2)} over {liveDaysToClose} days
                              </div>
                            </div>
                          </div>
                        }
                      >
                        <span className="spread-dollars">${liveNetSpread.toFixed(2)}</span>
                      </TearsheetTooltip>
                    </td>
                    <td className="spread-cell">
                      <span className="spread-pct">{liveNetPct.toFixed(2)}%</span>
                    </td>
                    {expandedColumnGroups.has('net') && (
                      <>
                        <td className="cost-cell">
                          <span className="cost-value">${liveBorrowCost.toFixed(2)}</span>
                        </td>
                        <td className="cost-cell">
                          <span className="cost-value">${deal.dividend_expected.toFixed(2)}</span>
                        </td>
                        <td className="spread-cell">
                          <span className="spread-pct">{liveAnnualizedNet.toFixed(2)}%</span>
                        </td>
                      </>
                    )}
                      ...
                      <TearsheetTooltip
                        ...
                        content={
                          <div>
                            <div className="tooltip-section">
                              <div className="tooltip-row">
                                <span className="tooltip-label">Announcement Date</span>
                                <span className="tooltip-value">
                                  {new Date(deal.announce_date).toLocaleDateString()}
                                </span>
                              </div>
                              <div className="tooltip-row">
                                <span className="tooltip-label">Closing Guidance</span>
                                <span className="tooltip-value">
                                  {deal.outside_date ? new Date(deal.outside_date).toLocaleDateString() : '—'}
                                </span>
                              </div>
                              <div className="tooltip-row">
                                <span className="tooltip-label">Our Est. Close</span>
                                <span className="tooltip-value">
                            ...
                            <div className="tooltip-section">
                              <div className="tooltip-section-title">Regulatory Timeline</div>
                              <div className="tooltip-row">
                                <span className="tooltip-label">HSR Filed Date</span>
                                <span className="tooltip-value">—</span>
                              </div>
                              <div className="tooltip-row">
                                <span className="tooltip-label">FTC Second Request Date</span>
                                <span className="tooltip-value">—</span>
                              </div>
                              <div className="tooltip-row">
                                <span className="tooltip-label">Timing Agreement Entered</span>
                                <span className="tooltip-value">—</span>
                              </div>
                              <div className="tooltip-row">
                                <span className="tooltip-label">Timing Agreement Expired</span>
                                <span className="tooltip-value">—</span>
                              </div>
                              <div className="tooltip-row">
                                <span className="tooltip-label">FTC Lawsuit Filed</span>
                                <span className="tooltip-value">—</span>
                              </div>
                              <div className="tooltip-row">
                                <span className="tooltip-label">Companies Response Filed</span>
                                <span className="tooltip-value">—</span>
                              </div>
                              <div className="tooltip-row">
                                <span className="tooltip-label">Deadline for Depositions</span>
                                <span className="tooltip-value">—</span>
                              </div>
                            </div>
                          </div>
                      ...
                      >
                        <span className="date-text">
                          {prData?.expected_close_date
                            ? new Date(prData.expected_close_date).toLocaleDateString()
                            : new Date(deal.expected_close).toLocaleDateString()}
                        </span>
                      </TearsheetTooltip>
                    </td>
                    ...
                    <td className="downside-cell">
                        ...
                        content={
                          <div>
                            <div className="tooltip-section">
                              ...
                              <div className="tooltip-row">
                                <span className="tooltip-label">Break Price</span>
                                <span className="tooltip-value">${breakPrice.toFixed(2)}</span>
                              </div>
                              <div className="tooltip-row">
                                <span className="tooltip-label">$ Downside</span>
                                <span className="tooltip-value">${(breakPrice - deal.current_price).toFixed(2)}</span>
                              </div>
                            </div>
                            <div className="tooltip-section">
                              <div className="tooltip-section-title">Beta-Adjusted Break</div>
                              <div className="tooltip-row">
                                <span className="tooltip-label">Undisturbed</span>
                                <span className="tooltip-value">${deal.unaffected_price.toFixed(2)}</span>
                              </div>
                              <div className="tooltip-row">
                                <span className="tooltip-label">SPY Move</span>
                                <span className="tooltip-value">{betaMultiplier !== 1 ? `${((betaMultiplier - 1) * 100).toFixed(1)}%` : '—'}</span>
                              </div>
                              <div className="tooltip-text" style={{fontSize: '11px', fontStyle: 'italic'}}>
                                Break = Undisturbed × (SPY now / SPY at announce)
                              </div>
                            </div>
                          </div>
                        }
                      >
                        {breakPrice > 0 ? (
                          <span className={`downside-value ${downsidePct < -30 ? 'high-risk' : ''}`}>
                          ...
                          <div>
                            <div className="tooltip-section">
                              <div className="tooltip-row">
                                ...
                                <span className="tooltip-value" style={{fontSize: '10px'}}>(Current − Break) / (Offer − Break)</span>
                              </div>
                              <div className="tooltip-row">
                                <span className="tooltip-label">Current Price</span>
                                <span className="tooltip-value">${deal.current_price.toFixed(2)}</span>
                            ...
                            <div className="tooltip-section">
                              <div className="tooltip-section-title">Beta Adjustment</div>
                              <div className="tooltip-row">
                                <span className="tooltip-label">Undisturbed</span>
                                <span className="tooltip-value">${deal.unaffected_price.toFixed(2)}</span>
                              </div>
                              ...
                              <div className="tooltip-text" style={{fontSize: '11px', fontStyle: 'italic'}}>
                                Break = Undisturbed × (SPY now / SPY at announce). Clamped to 0–100%.
                              </div>
                            </div>
                          </div>
                        }
                      >
                        <span className={`spread-pct ${ipc >= 90 ? 'spread-tight' : ipc >= 70 ? 'spread-mid' : 'spread-wide'}`}>
                    ...
                    {expandedColumnGroups.has('downsides') && (
                      <>
                        <td className="price-cell">
                          {breakPrice > 0
                            ? <span className="price-value">${breakPrice.toFixed(2)}</span>
                            : <span className="muted">—</span>
                          }
                        </td>
                      </>
            ...
            <div className="tearsheet-footer">
              <div className="footer-note">
                {liveQuotes?.timestamp ? (
                  <span className="muted">
                    📊 Quotes updated: {new Date(liveQuotes.timestamp).toLocaleTimeString()}
                    {liveQuotes.target_quote && ` • ${liveQuotes.target_quote.market_state}`}
                  </span>
                ) : (
                  <span className="muted">💡 Click "Refresh Quotes" to load live market data</span>
                )}
                {quotesError && (
                  <span className="error-text" style={{ marginLeft: '8px', color: 'var(--accent-red)' }}>
                    ⚠️ {quotesError}
                  </span>
                )}
              </div>
              <div className="footer-actions">
                ...
                >
                  <span>{quotesLoading ? '⏳ Loading...' : '🔄 Refresh Quotes'}</span>
                </button>
                <button className="tearsheet-action-btn" onClick={toggleFullScreen}>
        ...
        {activeTab === 'financial' && (
          <div className="financial-overview">
            ...
            <div className="financial-grid">
              <div className="fin-card">
                ...
                {isStockDeal ? (
                    ...
                    <div className="fin-row">
                      ...
                      <span className="fin-value fin-has-tooltip">
                        ...
                        <span className="fin-tooltip">
                          {acquirerPrice ? 'Live calculated' : 'Static — awaiting live quote'}
                        </span>
                      </span>
                    </div>
                    {deal.cash_per_share > 0 && (
                      <div className="fin-row" style={{ paddingLeft: '12px' }}>
                        <span className="fin-label" style={{ fontSize: '11px', color: 'var(--text-muted)' }}>Cash</span>
                        <span className="fin-value" style={{ fontSize: '11px' }}>${deal.cash_per_share.toFixed(2)}</span>
                      </div>
                    )}
                    <div className="fin-row" style={{ paddingLeft: '12px' }}>
                      <span className="fin-label" style={{ fontSize: '11px', color: 'var(--text-muted)' }}>Stock</span>
                      <span className="fin-value fin-has-tooltip" style={{ fontSize: '11px' }}>
                        {acquirerPrice
                        ...
                        <span className="fin-tooltip">Exchange ratio × acquirer live price</span>
                      </span>
                    </div>
                    {deal.cvr_per_share > 0 && (
                      <div className="fin-row" style={{ paddingLeft: '12px' }}>
                        <span className="fin-label" style={{ fontSize: '11px', color: 'var(--text-muted)' }}>CVR</span>
                        <span className="fin-value" style={{ fontSize: '11px' }}>${deal.cvr_per_share.toFixed(2)}</span>
                      </div>
                    )}
                    {deal.special_div > 0 && (
                      <div className="fin-row" style={{ paddingLeft: '12px' }}>
                        <span className="fin-label" style={{ fontSize: '11px', color: 'var(--text-muted)' }}>Special Div</span>
                        <span className="fin-value" style={{ fontSize: '11px' }}>${deal.special_div.toFixed(2)}</span>
                      </div>
                    )}
                  </>
                ) : (
                  <>
                    <div className="fin-row">
                      ...
                      <span className="fin-value fin-has-tooltip">
                        ${prData?.offer_price_cash != null ? prData.offer_price_cash.toFixed(2) : deal.offer_price.toFixed(2)}
                        <span className="fin-tooltip">
                          Source: {prData?.offer_price_cash != null ? 'Press Release' : 'deals.json'}
                        </span>
                      </span>
                    </div>
                    {prData?.cvr_value != null && prData.cvr_value > 0 && (
                      <div className="fin-row" style={{ paddingLeft: '12px' }}>
                        <span className="fin-label" style={{ fontSize: '11px', color: 'var(--text-muted)' }}>CVR</span>
                        <span className="fin-value" style={{ fontSize: '11px' }}>up to ${prData.cvr_value.toFixed(2)}</span>
                      </div>
                    )}
                  </>
                )}
                <div className="fin-row">
                  ...
                  <span className="fin-value">${deal.current_price.toFixed(2)}</span>
                </div>
                <div className="fin-row">
                  <span className="fin-label">Unaffected Price</span>
                  <span className="fin-value fin-has-tooltip">
                    ${unaffectedYf ? unaffectedYf.price.toFixed(2) : deal.unaffected_price.toFixed(2)}
                    <span className="fin-tooltip">
                      {unaffectedYf
                        ? `yfinance close on ${unaffectedYf.date}`
                        : 'deals.json (manual)'
                      }
                      {prData?.undisturbed_date && ` · Undisturbed: ${prData.undisturbed_date}`}
                      {prData?.undisturbed_reference && ` · Ref: ${prData.undisturbed_reference}`}
                    </span>
                  </span>
                </div>
              </div>
              ...
              <div className="fin-card">
                ...
                <div className="fin-row highlight">
                  ...
                  <span className={`fin-value ${liveGrossSpread >= 0 ? 'positive' : 'negative'}`}>
                  ...
                  {editingBorrow ? (
                    <div className="fin-spreads-edit">
                      <div className="fin-spread-row">
                        ...
                        <input type="number" step="5" value={longSpreadInput} onChange={e => setLongSpreadInput(e.target.value)} className="fin-inline-input" style={{ width: '50px' }} />
                        <span className="fin-spread-unit">bps over SOFR</span>
                      </div>
                      {isStockDeal && (
                        <div className="fin-spread-row">
                          <span className="fin-spread-label">Short spread</span>
                          <input type="number" step="5" value={shortSpreadInput} onChange={e => setShortSpreadInput(e.target.value)} className="fin-inline-input" style={{ width: '50px' }} />
                          <span className="fin-spread-unit">bps under SOFR</span>
                        </div>
                      )}
                      <div className="fin-spread-row">
                        <span className="fin-spread-label" style={{ color: 'var(--text-muted)' }}>SOFR: {(sofr * 100).toFixed(2)}%{sofrDate && ` (${sofrDate})`}</span>
                        <button className="fin-inline-save" onClick={saveSpreads}>OK</button>
                      </div>
                    </div>
                  ) : (
                    <span className="fin-value negative">
                      -${liveBorrowCost.toFixed(2)} <span style={{ fontSize: '10px', color: 'var(--text-muted)' }}>{useLegacyBorrow ? `(${(dealOverrides.borrow_rate_annual * 100).toFixed(1)}% ann.)` : `(${(longRate * 100).toFixed(2)}% long${isStockDeal ? ` / ${(shortRebateRate * 100).toFixed(2)}% rebate` : ''})`}</span>
                    </span>
                  )}
                </div>
                {isStockDeal && !editingBorrow && !useLegacyBorrow && (
                  <>
                    <div className="fin-row" style={{ paddingLeft: '12px' }}>
                      <span className="fin-label" style={{ fontSize: '10px' }}>Long Financing</span>
                      <span className="fin-value negative" style={{ fontSize: '10px' }}>-${longCost.toFixed(2)}</span>
                    </div>
                    <div className="fin-row" style={{ paddingLeft: '12px' }}>
                      <span className="fin-label" style={{ fontSize: '10px' }}>Short Rebate</span>
                      <span className="fin-value positive" style={{ fontSize: '10px' }}>+${shortRebate.toFixed(2)}</span>
                    </div>
                  </>
                )}
                <div className="fin-row">
                ...
                <div className="fin-row highlight">
                  ...
                  <span className={`fin-value ${liveNetSpread >= 0 ? 'positive' : 'negative'}`}>
                    ${liveNetSpread.toFixed(2)} ({liveNetPct.toFixed(1)}%)
                  </span>
                </div>
                <div className="fin-row highlight">
                  <span className="fin-label">Unlevered Annualized Return</span>
                  <span className={`fin-value ${liveAnnualizedNet >= 0 ? 'positive' : 'negative'}`}>{liveAnnualizedNet.toFixed(1)}%</span>
                </div>
              </div>
              ...
              <div className="fin-card">
                ...
                <div className="fin-row">
                  ...
                  <span className="fin-value">{formatDate(prData?.announce_date || deal.announce_date)}</span>
                </div>
                <div className="fin-row">
                  <span className="fin-label">Expected Close</span>
                  <span className="fin-value">
                    {prData?.expected_close || formatDate(deal.expected_close)}
                  </span>
                </div>
                <div className="fin-row">
                  <span className="fin-label">
                  ...
                  {editingClose ? (
                    <span className="fin-value" style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                      <input
                        type="date"
                        value={closeInput}
                        onChange={e => setCloseInput(e.target.value)}
                        onKeyDown={e => e.key === 'Enter' && saveExpectedClose()}
                        className="fin-inline-input"
                        style={{ width: '130px', textAlign: 'left' }}
                      />
                      <button className="fin-inline-save" onClick={saveExpectedClose}>OK</button>
                      {dealOverrides.expected_close_date && (
                        <button className="fin-inline-save" style={{ background: 'var(--text-muted)' }} onClick={resetExpectedClose}>RESET</button>
                      )}
                    </span>
                  ) : (
                    <span className="fin-value">
                      ...
                      {dealOverrides.expected_close_date
                        ? <span style={{ fontSize: '9px', color: 'var(--accent-blue)', marginLeft: '6px' }}>EDITED</span>
                        : <span style={{ fontSize: '9px', color: 'var(--text-muted)', marginLeft: '6px' }}>MIDPOINT</span>
                      }
                    </span>
                  )}
                </div>
                {deal.outside_date && (
                  <div className="fin-row">
                    ...
                    <span className="fin-value">{formatDate(deal.outside_date)}</span>
                  </div>
                )}
                <div className="fin-row">
                  <span className="fin-label">Days to Close</span>
                  <span className="fin-value">
                    {dealOverrides.expected_close_date ? '' : '~'}{liveDaysToClose}
                  </span>
                </div>
              </div>
              ...
              <div className="fin-card">
                ...
                <div className="fin-row">
                  ...
                  {prData?.diluted_shares_mm && prData?.total_consideration ? (
                    <span className="fin-value fin-calculated fin-has-tooltip">
                      ${((prData.diluted_shares_mm * prData.total_consideration) / 1000).toFixed(1)}B
                      <span className="fin-tooltip">
                        Calculated: {prData.diluted_shares_mm}M x ${prData.total_consideration}/sh
                        <br/>Source: Press Release
                      </span>
                    </span>
                  ) : prData?.deal_value_bn && prData?.cash_on_hand_bn != null && prData?.debt_bn != null ? (
                    <span className="fin-value fin-calculated fin-has-tooltip">
                      ${(prData.deal_value_bn - prData.debt_bn + prData.cash_on_hand_bn).toFixed(1)}B
                      <span className="fin-tooltip">
                        Calculated: EV ${prData.deal_value_bn}B - Debt ${prData.debt_bn}B + Cash ${prData.cash_on_hand_bn}B
                        <br/>Source: Press Release
                      </span>
                    </span>
                    ...
                    <span className="fin-value fin-has-tooltip">
                      ...
                      <span className="fin-tooltip">Enterprise Value from Press Release (equity not derivable)</span>
                    </span>
                  ) : (
                    <span className="fin-value">
                      {liveDealValueBn ? `$${liveDealValueBn.toFixed(1)}B` : '—'}
                ...
                <div className="fin-row">
                  ...
                  <span className="fin-value">{prData?.deal_type || deal.deal_type}</span>
                </div>
                <div className="fin-row">
            ...
            <div className="fin-card" style={{ marginTop: 'var(--space-lg)' }}>
              <div className="fin-card-title" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                {prSuccess && <span className="sec-process-success">{prSuccess}</span>}
                {prError && <span className="sec-process-error">{prError}</span>}
              </div>
                ...
                <div className="pr-extracted-grid">
                  {prData.offer_price_cash != null && (
                    <div className="fin-row">
                      ...
                      <span className="fin-value">${prData.offer_price_cash}</span>
                    </div>
                  )}
                  {prData.cvr_value != null && prData.cvr_value > 0 && (
                    <div className="fin-row">
                      <span className="fin-label">CVR</span>
                      <span className="fin-value">up to ${prData.cvr_value}</span>
                    </div>
                  )}
                  {prData.total_consideration != null && (
                    <div className="fin-row">
                      <span className="fin-label">Total Consideration</span>
                      <span className="fin-value">${prData.total_consideration}</span>
                    </div>
                  )}
                  {prData.deal_value_bn != null && prData.deal_value_bn > 0 && (
                    <div className="fin-row">
                      <span className="fin-label">Enterprise Value</span>
                      <span className="fin-value">${prData.deal_value_bn}B</span>
                    </div>
                  )}
                  {prData.deal_type && (
                    <div className="fin-row">
                  ...
                  {prData.premium_pct != null && (
                    <div className="fin-row">
                      <span className="fin-label">Premium</span>
                      <span className="fin-value positive">{prData.premium_pct}%</span>
                    </div>
                  )}
                  {prData.undisturbed_date && (
                    <div className="fin-row">
                      <span className="fin-label">Undisturbed Date</span>
                      <span className="fin-value">{formatDate(prData.undisturbed_date)}</span>
                    </div>
                  )}
                  {prData.expected_close && (
                    <div className="fin-row">
                      <span className="fin-label">Expected Close</span>
                      <span className="fin-value">{prData.expected_close}</span>
                    </div>
                  )}
                  {prData.go_shop_days != null && (
                    <div className="fin-row">
                      <span className="fin-label">Go-Shop Period</span>
                      <span className="fin-value">{prData.go_shop_days} days</span>
                    </div>
                  )}
                  {prData.financing && (
                    <div className="fin-row">
                  ...
                  {regData && regData.approvals && regData.approvals.length > 0 ? (
                          ...
                          {regData.approvals
                            ...
                            .map((a: any) => {
                              const statusCls: Record<string, string> = {
                                cleared: 'fin-reg-cleared', cleared_with_conditions: 'fin-reg-cleared',
                                filed: 'fin-reg-filed', under_review: 'fin-reg-filed',
                                phase2: 'fin-reg-phase2', blocked: 'fin-reg-blocked',
                                pending: 'fin-reg-pending', filing_intent: 'fin-reg-pending',
                              };
                              const statusLabel: Record<string, string> = {
                                cleared: 'CLEARED', cleared_with_conditions: 'CLEARED*',
                                filed: 'FILED', under_review: 'REVIEW',
                                phase2: 'PHASE 2', blocked: 'BLOCKED',
                                pending: 'PENDING', filing_intent: 'INTENT',
                              };
                              return (
                                <span key={a.id} className={`fin-reg-item ${statusCls[a.status] || 'fin-reg-pending'}`}>
                                  {a.authority_short}
                                  <span className="fin-reg-status">{statusLabel[a.status] || a.status.toUpperCase()}</span>
                                </span>
                              );
                            })}
                        </span>
                  ...
                  ) : prData.regulatory_bodies && prData.regulatory_bodies.length > 0 ? (
                    <div className="fin-row">
                      <span className="fin-label">Regulatory</span>
                      <span className="fin-value">{prData.regulatory_bodies.join(' · ')}</span>
                    </div>
                  ) : null}
                  {prData.special_conditions && (
                    <div className="fin-row">
                      <span className="fin-label">Special Terms</span>
                      <span className="fin-value fin-value-wrap">{prData.special_conditions}</span>
                    </div>
                  )}
                  {prData.dividend_info && prData.dividend_info !== 'null' && (
                    <div className="fin-row">
                      <span className="fin-label">Dividend Info</span>
                      <span className="fin-value">{prData.dividend_info}</span>
                    </div>
                  )}
                </div>
              )}
            ...
            <div className="fin-card" style={{ marginTop: 'var(--space-lg)' }}>
              <div className="fin-card-title" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                {dmaExSuccess && <span className="sec-process-success">{dmaExSuccess}</span>}
                {dmaExError && <span className="sec-process-error">{dmaExError}</span>}
              </div>
              ...
              {dmaInconsistencies.length > 0 && (
                <div className="dma-inconsistencies">
                  <div className="inconsistency-header">Inconsistencies vs Press Release</div>
                ...
                <div className="pr-extracted-grid">
                    ...
                    <div className="fin-row">
                      ...
                      <span className="fin-value">
                        ...
                        {dmaExtract.outside_date_extension && (
                          <span style={{ color: 'var(--text-muted)', fontWeight: 400 }}> / {formatDate(dmaExtract.outside_date_extension)}</span>
                        )}
                      </span>
                    </div>
                  )}
                  {dmaExtract.outside_date_extension_condition && (
                    <div className="fin-row">
                      ...
                      <span className="fin-value fin-value-wrap">{dmaExtract.outside_date_extension_condition}</span>
                    </div>
                  )}
                  {dmaExtract.target_break_fee_mm != null && (
                    <div className="fin-row">
                      <span className="fin-label">Target Break Fee</span>
                      <span className="fin-value">${dmaExtract.target_break_fee_mm}M</span>
                    </div>
                  )}
                  {dmaExtract.acquirer_reverse_break_fee_mm != null && (
                    <div className="fin-row">
                      <span className="fin-label">Reverse Break Fee</span>
                      <span className="fin-value">${dmaExtract.acquirer_reverse_break_fee_mm}M</span>
                    </div>
                  )}
                  {dmaExtract.dividend_allowed && (
                    <div className="fin-row">
                      <span className="fin-label">Dividend Allowed</span>
                      <span className="fin-value fin-value-wrap">{dmaExtract.dividend_allowed}</span>
                    </div>
                  )}
                </div>
              )}
        ...
        {activeTab === 'dma' && (
          <div className="dma-container">
            {dmaSummaryLoading && (
              <div className="placeholder"><p>Loading DMA summary from MongoDB…</p></div>
            )}
            {!dmaSummaryLoading && ((dmaSummary?.concise_sections?.length > 0) ||
              ...
              <>
                ...
                <div className="dma-deal-summary">
                  ...
                  <div className="summary-item">
                    ...
                    <span className="summary-value">{deal.deal_type.toUpperCase()}</span>
                  </div>
                  <div className="summary-item">
                    <span className="summary-label">Consideration</span>
                    <span className="summary-value">${deal.offer_price.toFixed(2)} per share</span>
                  </div>
                </div>
                ...
                {(() => {
                  ...
                  const sectionsToDisplay = (dmaSummary?.concise_sections && dmaSummary?.fulsome_sections)
                    ? (dmaViewMode === 'concise' ? dmaSummary.concise_sections : dmaSummary.fulsome_sections)
                    : (deal.concise_sections && deal.fulsome_sections)
                      ? (dmaViewMode === 'concise' ? deal.concise_sections : deal.fulsome_sections)
                      : deal.dma_sections || [];

                  return sectionsToDisplay.map((section: any, sectionIdx: number) => {
                    // Filter clauses based on search
                    const filteredClauses = section.clauses.filter((clause: any) => {
                      if (!dmaSearchQuery) return true;
                      const query = dmaSearchQuery.toLowerCase();
                      const clauseText = clause.text || clause.concise || clause.fulsome || clause.clause_text || '';
                      return (
                    ...
                    const showSectionClauses = expandedSections.has(sectionId);
                    ...
                    return (
                      <div key={sectionIdx} className="dma-section">
                        ...
                        >
                          <span className="section-toggle">{showSectionClauses ? '▾' : '▸'}</span>
                          {section.name}
                          <span className="clause-count">({filteredClauses.length})</span>
                        </h3>
                        {showSectionClauses && (
                          <div className="dma-clauses">
                            {filteredClauses.map((clause: any, clauseIdx: number) => {
                              const clauseId = `clause-${sectionIdx}-${clauseIdx}`;
                              const isExpanded = expandedClauses.has(clauseId);
                              const refs = clause.references && clause.references.length > 0
                                ? clause.references.join(', ')
                                : '—';
                              const fullText = clause.text || clause.concise || clause.fulsome || '';

                              return (
                                <div key={clauseId} className={`dma-clause ${isExpanded ? 'expanded' : ''}`}>
                                  <div className="clause-header" onClick={() => toggleClause(clauseId)}>
                                    <span className="clause-toggle">{isExpanded ? '▾' : '▸'}</span>
                                    <span className="clause-concise">{fullText}</span>
                                    <span className="clause-refs">{refs}</span>
                                  </div>
                                    ...
                                    <div className="clause-expanded">
                                      {clause.references && clause.references.length > 0 && (
                                        <div className="clause-references">
                                          <div className="layer-label">Document References</div>
                                          <div className="reference-tags">
                                            {clause.references.map((ref: string, i: number) => (
        ...
        {activeTab === 'timeline' && (
          <div className="dma-timeline-tab">
            ...
            {/* Document Sources */}
            <div className="fin-card" style={{ marginTop: 'var(--space-lg)', marginBottom: 'var(--space-md)' }}>
              ...
              <div className="doc-sources-grid">
                {docSources?.sources ? (
                  docSources.sources.map((src: any) => (
                    <div className="doc-source-row" key={src.source}>
                      ...
                      <span className={`doc-source-status ${src.status === 'loaded' ? 'status-loaded' : 'status-missing'}`}>
                        {src.status === 'loaded'
                          ? (src.count != null ? `${src.count} filing${src.count !== 1 ? 's' : ''}` : '✓ Loaded')
                          : '✗ Not loaded'}
                      </span>
                      ...
                      <span className="doc-source-updated">
                        {src.extracted_at ? formatDate(src.extracted_at.split('T')[0]) : ''}
                      </span>
                    </div>
                  ))
                ) : (
                  <>
                    <div className="doc-source-row">
                      ...
                      <span className={`doc-source-status ${prData ? 'status-loaded' : 'status-missing'}`}>
                        {prData ? '✓ Extracted' : '✗ Not loaded'}
                      </span>
                    </div>
                    <div className="doc-source-row">
                      <span className="doc-source-label">DMA Extract</span>
                      <span className={`doc-source-status ${dmaExtract ? 'status-loaded' : 'status-missing'}`}>
                ...
                <div className="doc-source-row">
                  ...
                  <span className={`doc-source-status ${(deal?.ai_sec_filings?.length || 0) > 0 ? 'status-loaded' : 'status-missing'}`}>
                    {(deal?.ai_sec_filings?.length || 0) > 0 ? `${deal!.ai_sec_filings!.length} filing${deal!.ai_sec_filings!.length > 1 ? 's' : ''}` : '✗ None'}
                  </span>
              ...
              {docSources?.discrepancies && docSources.discrepancies.length > 0 && (
                <div style={{ marginTop: 'var(--space-md)', paddingTop: 'var(--space-sm)', borderTop: '1px solid var(--border)' }}>
                  <div style={{ fontSize: '10px', fontWeight: 700, color: 'var(--accent-yellow)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '6px', fontFamily: 'var(--font-mono)' }}>
                    Cross-Source Discrepancies
                  </div>
                  {docSources.discrepancies.map((disc: any, i: number) => (
                    <div key={i} className="discrepancy-item">
                      <span className="discrepancy-field">{disc.label}</span>
                      <div className="discrepancy-sources">
                        {disc.sources.map((s: any, j: number) => (
                          <span key={j} className={`discrepancy-source-value ${s.source === disc.most_recent ? 'most-recent' : ''}`}>
                            <span className="discrepancy-source-name">
                              {s.source === 'press_release' ? 'PR' : s.source === 'dma_extract' ? 'DMA' : 'TL'}
                            </span>
                            {': '}
                            {typeof s.value === 'object' ? JSON.stringify(s.value) : String(s.value)}
                            {s.source === disc.most_recent && <span className="most-recent-badge">LATEST</span>}
                          </span>
                        ))}
                      </div>
                    </div>
            ...
            <div className="fin-card" style={{ marginTop: 'var(--space-lg)' }}>
              <div className="fin-card-title" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                {uploadSuccess && <span className="sec-process-success">{uploadSuccess}</span>}
                {uploadError && <span className="sec-process-error">{uploadError}</span>}
              </div>
              ...
              {timelineExpanded && (
                <div className="pr-submit-box">
                  ...
                  <button
                    ...
                    style={{ marginTop: '6px' }}
                  >
                    {uploadProcessing ? 'PROCESSING...' : (uploadDocType === 'proxy' || uploadDocType === 'tenk' ? 'SAVE' : 'EXTRACT')}
                  </button>
                </div>
              )}
            </div>
        ...
        {activeTab === 'proxy' && (
          <div className="sec-tab-wrapper">
            <div className="sec-split-wrapper" style={{height: 'calc(100vh - 300px)', minHeight: '400px'}}>
                          ...
                          >
                            <div className="feed-item-top">
                              <span
                                ...
                                style={{ backgroundColor: f.doc_type === 'summary' ? 'var(--accent-green)' : 'var(--accent-yellow)' }}
                              >
                                {f.filing_type}
                              </span>
                              <span className="proxy-doc-type-label" style={{
                                fontSize: '9px', fontWeight: 600, textTransform: 'uppercase',
                                color: f.doc_type === 'summary' ? 'var(--accent-green)' : 'var(--accent-yellow)',
                                fontFamily: 'var(--font-mono)'
                              }}>
                                {f.doc_type === 'summary' ? 'Summary' : 'Changes'}
                              </span>
                            </div>
                            ...
                            <div className="feed-item-meta">
                              ...
                              {f.generated && <span className="feed-item-date" style={{fontSize: '9px'}}>{f.generated}</span>}
                            </div>
                          </div>
                        );
                      })
                    ...
                    <>
                      <div className="sec-ai-detail-header">
                        <div className="sec-ai-detail-title">
                          <span
                            className="filing-type-badge"
                            style={{ backgroundColor: selectedProxy.doc_type === 'summary' ? 'var(--accent-green)' : 'var(--accent-yellow)' }}
                          >
                            {selectedProxy.filing_type}
                          </span>
                          <span className="detail-ticker">{selectedProxy.ticker}</span>
                          <span className="detail-date">{selectedProxy.generated}</span>
                        </div>
                      </div>
                      ...
                      {selectedProxy.doc_type === 'summary' && selectedProxy.detail_sections?.length > 0 && (
                        <div className="proxy-tabs-bar">
                          ...
                          {selectedProxy.detail_sections.map((sec: any) => (
                            <button
                              ...
                              onClick={() => setProxyDetailTab(`detail-${sec.number}`)}
                            >{sec.title}</button>
                          ))}
                        </div>
                      )}

                      <div className="sec-ai-detail-content" style={{padding: '16px 20px'}}>
                          ...
                          <>
                              ...
                              <>
                                {selectedProxy.qa_items?.length > 0 ? (
                                  <div className="proxy-qa-block">
                                    {selectedProxy.qa_items.map((qa: any, i: number) => (
                                      <div key={i} className="proxy-qa-item">
                                        <div className="proxy-qa-question">{qa.question}</div>
                                        <div className="proxy-qa-answer">{qa.answer}</div>
                                      </div>
                                    ))}
                                  </div>
                                ) : (
                                  <div style={{padding: '20px 0', color: 'var(--text-muted)', fontFamily: 'var(--font-mono)', fontSize: '11px'}}>
                                    No proxy summary Q&A available for this filing.
                                  </div>
                                )}

                                {selectedProxy.background && (
                                  <div className="proxy-background-block">
                                    <h5 className="l3-label">MERGER BACKGROUND ANALYSIS</h5>
                                    {selectedProxy.background.chronological_summary && (
                                      <p className="proxy-bg-summary">{selectedProxy.background.chronological_summary}</p>
                                    )}
                                    {selectedProxy.background.items?.length > 0 && (
                                      <ol className="proxy-bg-items">
                                        {selectedProxy.background.items.map((item: any, i: number) => (
                                          <li key={i} value={item.number} className="proxy-bg-item">
                                            {item.text}
                                          </li>
                                        ))}
                                      </ol>
                            ...
                            {selectedProxy.detail_sections?.map((sec: any) => (
                              proxyDetailTab === `detail-${sec.number}` && (
                                <div key={sec.number} className="proxy-detail-tab-content">
                        ...
                        {selectedProxy.doc_type === 'changes' && (
                          <>
                            {/* Per-section changes */}
                            {selectedProxy.sections?.map((section: any, si: number) => {
                              const collapseKey = `${selectedProxy.filename}-${section.name}`;
                              const isCollapsed = proxyCollapsed.has(collapseKey);
                              return (
                                <div key={si} className={`proxy-analysis-section ${!section.has_changes ? 'no-changes' : ''}`}>
                                  <div
                                    className="proxy-section-header"
                                    onClick={() => {
                                      const next = new Set(proxyCollapsed);
                                      isCollapsed ? next.delete(collapseKey) : next.add(collapseKey);
                                      setProxyCollapsed(next);
                                    }}
                                  >
                                    <h5 className="l3-label" style={{margin: 0, cursor: 'pointer'}}>
                                      <span style={{marginRight: '6px', fontSize: '8px'}}>{isCollapsed ? '▶' : '▼'}</span>
                                      {section.name}
                                      {!section.has_changes && <span className="proxy-no-changes-tag">No changes</span>}
                                    </h5>
                                  </div>
                                  {!isCollapsed && section.has_changes && (
                                    <div className="proxy-section-body">
                                      {section.raw_text && !section.items?.length ? (
                                        <p className="l3-text">{section.raw_text}</p>
                                      ) : (
                                        section.items?.map((item: any, ii: number) => (
                                          <div key={ii} className="proxy-change-item">
                                            <div className="proxy-item-header">
                                              {item.label && <span className="proxy-item-label">{item.label}</span>}
                                              {item.tag && <span className="proxy-new-tag">{item.tag}</span>}
                                            </div>
                                            {item.value && <div className="proxy-item-value">{item.value}</div>}
                                            {item.was && (
                                              <div className="proxy-was-now">
                                                <div className="proxy-was"><span>Was:</span> {item.was}</div>
                                                <div className="proxy-now"><span>Now:</span> {item.now}</div>
                                              </div>
                                            )}
                                          </div>
                                        ))
          ...
          <div className="sec-tab-wrapper">
            ...
            <div className="sec-url-bar">
              <input
                ...
                onKeyDown={(e) => { if (e.key === 'Enter' && !secBatchMode) handleSecProcess(); }}
              />
              <button
                onClick={handleSecProcess}
                disabled={secProcessing || (!secProcessUrl.trim() && !secBatchUrls.trim())}
                className="sec-process-btn"
              >
              ...
              {secProcessError && <span className="sec-process-error">{secProcessError}</span>}
              {secProcessSuccess && <span className="sec-process-success">{secProcessSuccess}</span>}
            </div>
            {secBatchMode && (
              <textarea
            ...
            <div className="sec-ai-split">
              ...
              <div className="sec-ai-left">
                <div className="sec-ai-feed-header">
                  <div className="sec-role-toggle">
                    <button
                      ...
                      onClick={() => { setSecFilingRole('target'); setSelectedSECFiling(null); setSecTypeFilter('all'); }}
                    >
                      {deal.target} ({deal.target_ticker})
                    </button>
                    <button
                      className={`sec-role-btn ${secFilingRole === 'acquirer' ? 'active' : ''}`}
                      onClick={() => { setSecFilingRole('acquirer'); setSelectedSECFiling(null); setSecTypeFilter('all'); }}
                    >
                  ...
                  {(() => {
                    const roleFilings = deal.ai_sec_filings?.filter((f: any) => f._role === secFilingRole) || [];
                    const types = [...new Set(roleFilings.map((f: any) => f.form_type))].sort();
                    return types.length > 1 ? (
                      <div className="sec-type-filters">
                        <button className={`sec-type-btn ${secTypeFilter === 'all' ? 'active' : ''}`} onClick={() => setSecTypeFilter('all')}>All</button>
                        {types.map((t: string) => (
                          <button key={t} className={`sec-type-btn ${secTypeFilter === t ? 'active' : ''}`} onClick={() => setSecTypeFilter(t)}>{t}</button>
                        ))}
                      </div>
                    ) : null;
                  })()}
                ...
                <div className="sec-ai-feed">
                  {(() => {
                    const roleFilings = deal.ai_sec_filings?.filter((f: any) => f._role === secFilingRole) || [];
                    const filtered = secTypeFilter === 'all' ? roleFilings : roleFilings.filter((f: any) => f.form_type === secTypeFilter);
                    const sorted = [...filtered].sort((a: any, b: any) => new Date(b.date).getTime() - new Date(a.date).getTime());
                    return sorted.length > 0 ? sorted.map((f: any, idx: number) => {
                      const isSelected = selectedSECFiling?.url === f.url;
                      const headline = f.summary?.L1_headline || f.form_type;
                      const person = f.summary?.insider_name || f.summary?.seller_name || f.summary?.filer_name || null;
                      return (
                        <div
                          key={`${f._slug}-${idx}`}
                          className={`sec-ai-feed-item ${isSelected ? 'selected' : ''}`}
                          onClick={() => { setSelectedSECFiling(f); setSecDetailView(secDefaultView); }}
                        >
                          <div className="feed-item-top">
                            <span
                              className="filing-type-badge"
                          ...
                          <div className="feed-item-headline">
                            {(headline || '').replace(/^\+\s*/, '').trim().slice(0, 80)}
                          </div>
                          <div className="feed-item-meta">
                            <span className="feed-item-ticker">{f.ticker}</span>
                      ...
                      <div className="sec-ai-empty">No SEC filings analyzed yet. Paste a URL above to get started.</div>
                    );
                  })()}
                </div>
              </div>
                  ...
                  <>
                    <div className="sec-ai-detail-header">
                      <div className="sec-ai-detail-title">
                        <span
                          className="filing-type-badge"
                          style={{ backgroundColor: getFilingTypeColor(selectedSECFiling.form_type) }}
                        >
                          {selectedSECFiling.form_type}
                        </span>
                        <span className="detail-ticker">{selectedSECFiling.ticker}</span>
                        <span className="detail-date">{formatDate(selectedSECFiling.date)}</span>
                      </div>
                      <button className="close-detail" onClick={() => setSelectedSECFiling(null)}>&#x2715;</button>
                    </div>

                    <div className="sec-ai-l1">
                      {selectedSECFiling.summary?.L1_headline || selectedSECFiling.form_type}
                    </div>

                    <div className="sec-ai-detail-tabs">
                      <button
                        className={secDetailView === 'summary' ? 'active' : ''}
                      ...
                      {secDetailView === 'summary' ? (
                          ...
                          {selectedSECFiling.summary?.items_reported && (
                            <div className="ai-meta-grid">
                              <div className="ai-meta-item">
                                ...
                                <span className="ai-meta-value">{selectedSECFiling.summary.items_reported.join(', ')}</span>
                              </div>
                              <div className="ai-meta-item">
                                <span className="ai-meta-label">Filed</span>
                                <span className="ai-meta-value">{selectedSECFiling.summary?.filing_date || formatDate(selectedSECFiling.date)}</span>
                              </div>
                            </div>
                          )}
                          {selectedSECFiling.summary?.L2_brief && (
                            <div className="ai-l2-brief">
                              <p className="l3-text">{selectedSECFiling.summary.L2_brief}</p>
                            </div>
                          )}
                        ...
                        <>
                          {selectedSECFiling.summary?.L3_detailed && (
                            renderL3Detail(selectedSECFiling.form_type, selectedSECFiling.summary.L3_detailed)
                          )}
                          {selectedSECFiling.url && (
                            <div style={{marginTop: '16px'}}>
                              <a
                    ...
                    {selectedSECFiling.url && ['8-K', '8-K/A'].includes(selectedSECFiling.form_type) && (
                      <div className="sec-run-termination-bar">
                        ...
                        >Run Termination from 8-K</button>
                        <span className="sec-run-hint">Extracts fee data from this filing</span>
                      </div>
                    )}
                  </>
                ) : (
          ...
          <div className="sec-tab-wrapper">
            <div className="sec-split-wrapper" style={{height: 'calc(100vh - 300px)', minHeight: '400px'}}>
              <div className="sec-ai-split">
                ...
                <div className="sec-ai-left">
                  <div className="sec-ai-feed-header">
                    <span className="sec-toggle-label" style={{marginRight: 'auto'}}>10-K / 10-Q Filings</span>
                    <button className="pr-toggle-btn" onClick={() => setTenkUploadOpen(!tenkUploadOpen)}>
                  ...
                  <div className="sec-ai-feed">
                    {tenkLoading ? (
                      <div className="sec-ai-empty">Loading 10-K/10-Q analyses...</div>
                    ) : tenkPeriods.length === 0 ? (
                      <div className="sec-ai-empty">No 10-K/10-Q analyses found.</div>
                    ) : (
                      tenkPeriods.map((group, idx) => {
                        ...
                        return (
                          ...
                          >
                            <div className="feed-item-top">
                              <span className="filing-type-badge" style={{
                                backgroundColor: group.filingType === '10-K' ? 'var(--accent-green)' : 'var(--accent-blue)'
                              }}>
                                {group.filingType}
                              </span>
                              {group.hasComparison ? (
                                <span style={{
                                  fontSize: '9px', fontWeight: 600, textTransform: 'uppercase',
                                  color: 'var(--accent-yellow)', fontFamily: 'var(--font-mono)'
                                }}>Updated</span>
                              ) : (
                                <span style={{
                                  fontSize: '9px', fontWeight: 600, textTransform: 'uppercase',
                            ...
                            <div className="feed-item-headline">{group.label}</div>
                            <div className="feed-item-meta">
                              <span className="feed-item-ticker">{(group.overview || group.exec || group.redline)?.ticker}</span>
                            </div>
                          </div>
                        );
                      })
                  ...
                  {selectedTenkGroup ? (
                    <>
                      <div className="sec-ai-detail-header">
                        <div className="sec-ai-detail-title">
                          <span className="filing-type-badge" style={{
                            backgroundColor: selectedTenkGroup.filingType === '10-K' ? 'var(--accent-green)' : 'var(--accent-blue)'
                          }}>
                            {selectedTenkGroup.filingType}
                          </span>
                          <span className="detail-ticker">{selectedTenkGroup.label}</span>
                        </div>
                        {/* Toggle moved to feed header */}
                      </div>

                      {/* Transition bar for comparison views */}
                      {selectedTenkGroup.hasComparison && (selectedTenkGroup.exec?.transition || selectedTenkGroup.redline?.transition) && (
                        <div className="proxy-transition-bar">
                          {selectedTenkGroup.exec?.transition || selectedTenkGroup.redline?.transition}
                        </div>
                      )}
                      ...
                      <div className="sec-ai-detail-content" style={{padding: '16px 20px'}}>
                        ...
                        {!selectedTenkGroup.hasComparison && selectedTenkGroup.overview && (
                          <>
                            {/* Headline (L1/L2/L3 format) */}
                            {selectedTenkGroup.overview.headline && (
                              <div className="tenk-headline">{selectedTenkGroup.overview.headline}</div>
                            )}

                            {/* Overview / Brief text */}
                            {selectedTenkGroup.overview.overview && (
                              <div className="tenk-overview-line">{selectedTenkGroup.overview.overview}</div>
                            )}

                            {/* Category sections with bullets (exec-format standalone) */}
                            {(selectedTenkGroup.overview.sections || []).length > 0 && (
                              <>
                                {(selectedTenkGroup.overview.sections || []).map((section: any, si: number) => (
                                  <div key={si} className="tenk-exec-section">
                                    <h5 className="l3-label">{section.name}</h5>
                                    <ul className="tenk-exec-items">
                                      {section.items.map((item: string, ii: number) => (
                                        <li key={ii} className="tenk-exec-item">{item}</li>
                                      ))}
                                    </ul>
                                  </div>
                                ))}
                            ...
                            {(selectedTenkGroup.overview.excerpts || []).length > 0 && (
                              <>
                                {(() => {
                                  const allTags: string[] = Array.from(new Set(
                                    (selectedTenkGroup.overview.excerpts || []).flatMap((e: any) => e.tags as string[])
                                  ));
                                  return (
                                    <div className="tenk-filter-bar">
                                      <button
                                        className={`tenk-filter-btn ${tenkTagFilter === 'ALL' ? 'active' : ''}`}
                                        onClick={() => setTenkTagFilter('ALL')}
                                      >ALL</button>
                                      {allTags.map((tag: string) => {
                                        const tagColor = tag === 'TIMING' ? 'var(--accent-blue)' : tag === 'REGULATORY' ? 'var(--accent-yellow)' : '#b794f4';
                                        const isActive = tenkTagFilter === tag;
                                        return (
                                          <button
                                            key={tag}
                                            className={`tenk-filter-btn ${isActive ? 'active' : ''}`}
                                            style={isActive ? { background: tagColor, borderColor: tagColor, color: 'var(--bg-primary)' } : { borderColor: tagColor, color: tagColor }}
                                            onClick={() => setTenkTagFilter(tag)}
                                          >{tag}</button>
                                        );
                                      })}
                                    </div>
                                ...
                                {(selectedTenkGroup.overview.excerpts || [])
                                  .filter((e: any) => tenkTagFilter === 'ALL' || e.tags.includes(tenkTagFilter))
                                  .map((excerpt: any) => (
                                    <div key={excerpt.number} className="tenk-excerpt">
                                      <div className="tenk-excerpt-header">
                                        <span className="tenk-excerpt-num">{excerpt.number}.</span>
                                        {excerpt.tags.map((tag: string) => (
                                          <span key={tag} className={`tenk-tag-badge ${tag === 'TIMING' ? 'tag-timing' : tag === 'REGULATORY' ? 'tag-regulatory' : 'tag-legal'}`}>{tag}</span>
                                        ))}
                                      </div>
                                      {excerpt.section && (
                                        <div className="tenk-section-source">{excerpt.section}</div>
                                      )}
                                      <div className="tenk-excerpt-text">{excerpt.text}</div>
                                    </div>
                                  ))}
                              </>
                            )}
                        ...
                        {selectedTenkGroup.hasComparison && tenkViewMode === 'summary' && selectedTenkGroup.exec && (
                          <>
                            {selectedTenkGroup.exec.overview && (
                              <div className="tenk-overview-line">{selectedTenkGroup.exec.overview}</div>
                            )}
                            {(selectedTenkGroup.exec.sections || []).map((section: any, si: number) => (
                              <div key={si} className="tenk-exec-section">
                                <h5 className="l3-label">{section.name}</h5>
                                <ul className="tenk-exec-items">
                                  {section.items.map((item: string, ii: number) => (
                        ...
                        {selectedTenkGroup.hasComparison && tenkViewMode === 'detail' && selectedTenkGroup.redline && (
                          <>
                            {selectedTenkGroup.redline.comparison_header && (
                              <div className="tenk-comparison-header">{selectedTenkGroup.redline.comparison_header}</div>
                            )}
                            {(selectedTenkGroup.redline.redline_excerpts || []).map((excerpt: any) => (
                              <div key={excerpt.number} className="tenk-redline-excerpt">
                                <div className="tenk-redline-header">
                                  <span className="tenk-excerpt-num">{excerpt.number}.</span>
                                  <span className={`tenk-significance ${excerpt.significance.toLowerCase()}`}>
                                  ...
                                  {excerpt.source && (
                                    <span className="tenk-redline-source">— {excerpt.source}</span>
                                  )}
                                </div>
                                ...
                                {excerpt.is_new ? (
                                  <div className="tenk-text-block tenk-new-disclosure">
                                    <div className="tenk-text-label">New Disclosure — {excerpt.current_label}</div>
                                    <div className="tenk-text-body">{excerpt.current_text}</div>
                                  </div>
                                ) : (
                                  <div className="tenk-text-comparison">
                                    <div className="tenk-text-block tenk-current">
                                      <div className="tenk-text-label">Current: {excerpt.current_label}</div>
                                      <div className="tenk-text-body">{excerpt.current_text}</div>
                                    </div>
                                    <div className="tenk-text-block tenk-prior">
                                      <div className="tenk-text-label">Prior: {excerpt.prior_label}</div>
                                      <div className="tenk-text-body">{excerpt.prior_text}</div>
                                    </div>
                                  </div>
                                )}
                                ...
                                {excerpt.phrase_changes?.length > 0 && (
                                  <div className="tenk-phrase-changes">
                                    <div className="tenk-phrase-header">Specific Phrase Changes</div>
                                    {excerpt.phrase_changes.map((pc: any, pi: number) => (
                                      <div key={pi} className="tenk-phrase-pair">
                                        <div className="tenk-phrase-row">
                                          <div className="tenk-phrase-current">
                                            <span className="tenk-phrase-label">Current</span>
                                            <div className="tenk-phrase-text">{pc.current}</div>
                                          </div>
                                          {pc.prior && (
                                            <div className="tenk-phrase-prior">
                                              <span className="tenk-phrase-label">Prior</span>
                                              <div className="tenk-phrase-text">{pc.prior}</div>
                                            </div>
                                          )}
                                        </div>
                                        {pc.analysis && (
                                          <div className="tenk-phrase-analysis">{pc.analysis}</div>
                                        )}
                                      </div>
                                    ))}
                                  </div>
                                ...
                                {excerpt.category_analysis?.length > 0 && (
                                  <div className="tenk-cat-analysis">
                                    {excerpt.category_analysis.map((ca: any, ci: number) => (
                                      <div key={ci} className="tenk-cat-item">
                                        <span className="tenk-cat-label">{ca.category}:</span>
                                        <span className="tenk-cat-text">{ca.text}</span>
                                      </div>
                                    ))}
                                  </div>
                                )}
        ...
        {activeTab === 'mae' && (
          <div className="dma-timeline-tab">
            {/* Sub-view toggle */}
            <div className="mae-view-toggle" style={{ display: 'flex', gap: '8px', padding: '12px 16px', borderBottom: '1px solid var(--border)' }}>
              <button
                className={maeView === 'analysis' ? 'tab-btn active' : 'tab-btn'}
                onClick={() => setMaeView('analysis')}
              >Clause Analysis</button>
              <button
                className={maeView === 'pipeline' ? 'tab-btn active' : 'tab-btn'}
            ...
            {maeView === 'analysis' && (
              <>
                {maeDataLoading && (
                  <div className="placeholder"><p>Loading MAE analysis from MongoDB…</p></div>
                )}
                {!maeDataLoading && maeData && (
                  <MAEReview
                    dealName={maeData.deal_name || `${deal.target} / ${deal.acquirer}`}
                ...
                {!maeDataLoading && !maeData && (
                  <div className="placeholder">
                    ...
                    <p style={{ fontSize: '0.85em', color: 'var(--text-muted)' }}>
                      MAE analysis is sourced from the <code>mae_analyses</code> collection. Switch to Pipeline Dashboard to generate one.
                    </p>
                  </div>
                )}
              </>
            ...
            {maeView === 'pipeline' && (
              <>
                {maeStatus === 'checking' && (
                  <div className="placeholder"><p>Checking for MAE pipeline output...</p></div>
                )}
                {maeStatus === 'ready' && (
                  <iframe
                    key={maeStatus}
                ...
                {maeStatus === 'running' && (
                  <div className="pipeline-run-card">
                    <div className="pipeline-progress">Running MAE pipeline...</div>
                    <p className="step-label">Step: {maePipelineStep || 'starting'}</p>
                  </div>
                )}
                {maeStatus === 'error' && (
                  <div className="pipeline-run-card">
                    ...
                    {mergerUrlSaved ? (
                      <>
                            ...
                            fetch(`${API_BASE_URL}/api/deals/${dealId}/mae/run-pipeline`, { method: 'POST' })
                              ...
                              .then(data => {
                                if (data.status === 'already_running') {
                                  setMaePipelineStep(data.step || 'in progress');
                                }
                              })
                              .catch(() => {
                                setMaeStatus('error');
                        ...
                        >Run MAE Analysis</button>
                        <p>~8 min — extracts MAE clause, classifies against benchmark, runs compliance checks</p>
                      </>
                    ) : (
                      <div className="pipeline-no-url">
                        <p>Set the merger agreement URL in the Documents tab to enable MAE analysis.</p>
                        <button className="retry-btn" onClick={() => setMaeStatus('idle')}>Retry</button>
                      </div>
                    )}
                  </div>
                )}
        ...
        {activeTab === 'covenants' && (
          <div className="dma-timeline-tab">
            {covenantStatus === 'checking' && (
              <div className="placeholder"><p>Checking for covenant analysis…</p></div>
            )}
            {covenantStatus === 'generating' && (
              <div className="placeholder">
                <p>⏳ Generating covenant dashboard…</p>
                <p className="muted">Drop stage JSONs in backend/data/covenants/input/{dealId}/ to enable auto-generation.</p>
              </div>
            )}
            ...
            {covenantStatus === 'error' && covenantPipelineStatus === 'running' && (
              <div className="pipeline-run-card">
                <div className="pipeline-progress">Running covenant pipeline...</div>
                <p className="step-label">Step: {covenantPipelineStep || 'starting'}</p>
              </div>
            )}
            {covenantStatus === 'error' && covenantPipelineStatus !== 'running' && (
              <div className="pipeline-run-card">
                {covenantPipelineStatus === 'error' && covenantError && (
                  <p style={{ color: 'var(--accent-red)' }}>{covenantError}</p>
                )}
                {mergerUrlSaved ? (
                  <>
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        fetch(`${API_BASE_URL}/api/deals/${dealId}/covenants/run-pipeline`, { method: 'POST' })
                          ...
                          .then(data => {
                            if (data.status === 'already_running') {
                              setCovenantPipelineStep(data.step || 'in progress');
                            }
                          })
                          .catch(() => {
                            setCovenantPipelineStatus('error');
                    ...
                    >Run Covenant Analysis</button>
                    <p>~10 min — scrapes merger agreement, classifies clauses, generates dashboard</p>
                  </>
                ) : (
                  <div className="pipeline-no-url">
                    <p>Set the merger agreement URL in the Documents tab to enable covenant analysis.</p>
                    <button className="retry-btn" onClick={() => setCovenantStatus('idle')}>Retry</button>
                  </div>
                )}
              </div>
            )}
        ...
        {activeTab === 'regulatory' && (
          <div className="regulatory-wrapper">
            <RegulatoryTab dealId={dealId!} onProcessed={refreshDependentData} />
          </div>
        )}
        ...
        {activeTab === 'reg-monitor' && (
          <div className="regulatory-wrapper">
            <RegulatoryMonitorTab
              dealId={dealId!}
              dealName={deal ? `${deal.target}${deal.target_ticker ? ` (${deal.target_ticker})` : ''} / ${deal.acquirer}` : undefined}
            ...
            {deal.docket_entries && deal.docket_entries.length > 0 ? (
              ...
              <div className="content-panel">
                ...
                <div className="info-message">
                  ...
                  <p className="muted">Docket analysis will be added for deals with regulatory proceedings or litigation.</p>
                </div>
              </div>
            )}
          </div>
        ...
        {activeTab === 'twitter' && (
          <div className="content-panel">
            ...
            <div className="info-message">
              <p>🐦 Social media sentiment from Twitter/X, including posts from investors, analysts, and financial news.</p>
              ...
              <p className="muted">Twitter API integration coming soon for real-time mentions and sentiment.</p>
            </div>
          </div>
        )}
        ...
        {activeTab === 'termination' && (
          <div className="dma-timeline-tab">
            {terminationStatus === 'checking' && (
              <div className="placeholder"><p>Checking for termination analysis…</p></div>
            )}
            {terminationStatus === 'ready' && (
              <>
                ...
                {terminationHtml ? (
                  ...
                  <div className="placeholder"><p>Loading termination dashboard...</p></div>
                )}
                  ...
                  <div className="termination-audit-footer">
                    ...
                    {terminationSources.map((s: any, i: number) => (
                      <span key={i} className="audit-source-tag">
                        {s.type}
                        {s.doc_id && <span className="audit-doc-id">({s.doc_id})</span>}
                        <span className="audit-ts">{new Date(s.timestamp).toLocaleDateString('en-US', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</span>
                      </span>
                    ))}
                  </div>
                )}
            ...
            {terminationStatus === 'error' && terminationPipelineStatus === 'running' && (
              <div className="pipeline-run-card">
                <div className="pipeline-progress">Running termination pipeline...</div>
                <p className="step-label">Step: {terminationPipelineStep || 'starting'}</p>
              </div>
            )}
            {terminationStatus === 'error' && terminationPipelineStatus !== 'running' && (
              <div className="pipeline-run-card">
                {terminationPipelineStatus === 'error' && terminationError && (
                  <p style={{ color: 'var(--accent-red)' }}>{terminationError}</p>
                )}
                {mergerUrlSaved && (
                  <>
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        fetch(`${API_BASE_URL}/api/deals/${dealId}/termination/run-pipeline`, { method: 'POST' })
                          ...
                          .then(data => {
                            if (data.status === 'already_running') {
                              setTerminationPipelineStep(data.step || 'in progress');
                            }
                          })
                          .catch(() => {
                            setTerminationPipelineStatus('error');
                    ...
                    >Run from Merger Agreement</button>
                    <p className="step-label">~12 min — scrapes merger agreement, classifies termination clauses, runs provision checks</p>
                  </>
                )}
                {!mergerUrlSaved && (
                  <p className="step-label">No merger agreement URL stored. Paste a URL below or set it in the Documents tab.</p>
                )}
                <div className="termination-alt-url">
                  <span className="step-label">Or run from a specific filing:</span>
                  <input
                    className="termination-url-input"
            ...
            {terminationStatus === 'idle' && (
              <div className="placeholder"><p>Loading…</p></div>
            )}
          </div>
        )}
        ...
        {activeTab === 'documents' && (
          <div className="documents-tab">
              ...
              {mergerUrlSaved ? (
                <div className="merger-url-saved-block">
                  <div className="merger-url-saved">
                    ...
                    <a href={mergerUrlSaved} target="_blank" rel="noopener noreferrer">
                      {mergerUrlSaved.length > 80 ? mergerUrlSaved.slice(0, 80) + '...' : mergerUrlSaved}
                    </a>
                    <button className="merger-url-btn" onClick={() => { setMergerUrlSaved(null); setMergerUrlInput(''); }}>CLEAR</button>
                  </div>
                  <div className="pipeline-checkboxes">
                    <label className="pipeline-checkbox-label">
                      <input type="checkbox" checked={runMaeOnSave} onChange={e => setRunMaeOnSave(e.target.checked)} />
                      MAE
                    </label>
                    <label className="pipeline-checkbox-label">
                      <input type="checkbox" checked={runCovenantsOnSave} onChange={e => setRunCovenantsOnSave(e.target.checked)} />
                      Covenants
                    </label>
                    <label className="pipeline-checkbox-label">
                      <input type="checkbox" checked={runTerminationOnSave} onChange={e => setRunTerminationOnSave(e.target.checked)} />
                      Termination
                    </label>
                    <button className="merger-url-btn" disabled={!runMaeOnSave && !runCovenantsOnSave && !runTerminationOnSave} onClick={() => {
                      ...
                      setPipelineRunFeedback(`Started: ${started.join(', ')}`);
                      setTimeout(() => setPipelineRunFeedback(null), 5000);
                    }}>RUN</button>
                    {pipelineRunFeedback && <span className="pipeline-run-feedback">{pipelineRunFeedback}</span>}
                  </div>
                </div>
              ) : (
                <>
                      ...
                      onClick={() => {
                        fetch(`${API_BASE_URL}/api/deals/${dealId}/merger-agreement-url`, {
                          ...
                          .then(data => {
                            ...
                            if (data.pipelines_started?.includes('covenants')) {
                              setCovenantPipelineStatus('running'); setCovenantPipelineStep('starting'); setCovenantStatus('error');
                            }
                            if (data.pipelines_started?.includes('termination')) {
                              setTerminationPipelineStatus('running'); setTerminationPipelineStep('starting'); setTerminationStatus('error');
                            }
                          })
            ...
            <div className="fin-card">
              <div className="fin-card-title" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                Upload Document
                <button className="pr-toggle-btn" onClick={() => setTimelineExpanded(!timelineExpanded)}>
                  {timelineExpanded ? 'COLLAPSE' : 'ADD'}
                </button>
                {uploadSuccess && <span className="sec-process-success">{uploadSuccess}</span>}
                {uploadError && <span className="sec-process-error">{uploadError}</span>}
              ...
              {timelineExpanded && (
                <div className="pr-submit-box">
                  ...
                  <textarea
                    placeholder={
                      uploadDocType === 'dma_summary' ? 'Paste the DMA summary text here...' :
                      uploadDocType === 'press_release' ? 'Paste the press release text here...' :
                      uploadDocType === 'proxy' ? 'Paste proxy analysis summary here...' :
                  ...
                  />
                  <button
                    onClick={() => { handleUnifiedUpload(); setTimeout(refreshDocSources, 2000); }}
                    disabled={uploadProcessing || !uploadText.trim()}
                    className="sec-process-btn"
                    style={{ marginTop: '6px' }}
                  >
            ...
            <div className="fin-card" style={{ marginTop: 'var(--space-lg)' }}>
              ...
              {allDocsLoading ? (
                ...
                const uploadedTypes = ['press_release', 'dma_extract', 'proxy', 'tenk', 'sec_filings'];
                const uploaded = allDocSources.sources.filter((s: any) => uploadedTypes.includes(s.source));
                return (
                  <div className="docs-inventory">
                    ...
                    {uploaded.map((src: any) => (
                      <React.Fragment key={src.source}>
                        <div className={`docs-row ${src.status === 'missing' ? 'docs-row-missing' : ''} ${src.status === 'loaded' ? 'docs-row-clickable' : ''}`}
                             onClick={() => src.status === 'loaded' && toggleDocPreview(src.source)}>
                          <span className="docs-col-type">
                            {src.status === 'loaded' && <span className="docs-expand-icon">{expandedDocs.has(src.source) ? '▾' : '▸'}</span>}
                            {src.label}
                          </span>
                          <span className={`docs-col-status ${src.status === 'loaded' ? 'docs-status-loaded' : 'docs-status-missing'}`}>
                          ...
                          <span className="docs-col-processed">
                            {src.extracted_at
                              ...
                              : src.file_modified
                                ? formatDate(src.file_modified.split('T')[0])
                                : '—'}
                          </span>
                          <span className="docs-col-detail">
                            {src.count != null ? `${src.count} item${src.count !== 1 ? 's' : ''}` : ''}
                            {src.filled != null ? ` (${src.filled} filled)` : ''}
                          </span>
                          <span className="docs-col-actions" onClick={(e) => e.stopPropagation()}>
                            {src.status === 'loaded' && ['dma_extract', 'press_release'].includes(src.source) && (
                              <button className="docs-action-btn" onClick={() => handleViewSourceText(src.source)}>SRC</button>
                            )}
                            {src.status === 'loaded' && (
                              <button className="docs-action-btn docs-action-delete" onClick={() => handleDeleteDocument(src.source)}>DEL</button>
                            )}
                          </span>
                        </div>
                        {/* Expandable preview for single-file types */}
                        {expandedDocs.has(src.source) && !src.files?.length && (
                          <div className="docs-preview">
                            {!docPreviews[src.source] ? (
                              <div className="docs-preview-loading">Loading...</div>
                            ) : (
                              <div className="docs-preview-fields">
                                {docPreviews[src.source].fields.map((f: any, i: number) => (
                                  <div key={i} className="docs-preview-field">
                                    <span className="docs-preview-label">{f.label}</span>
                                    <span className="docs-preview-value">{f.value}</span>
                                  </div>
                                ))}
                              </div>
                            )}
                        ...
                        {expandedDocs.has(src.source) && src.files?.length > 0 && (
                          <div className="docs-file-group">
                            {src.files.map((f: any) => (
                              <div key={f.filename} className="docs-file-row">
                                <span className="docs-file-name">{f.form_type ? `${f.form_type} — ${f.filename}` : f.filename}</span>
                                <span className="docs-file-size">{f.size_kb ? `${f.size_kb} KB` : f.ticker || ''}</span>
                                <span className="docs-file-date">{f.modified ? formatDate(f.modified.split('T')[0]) : f.date || '—'}</span>
                                <span className="docs-file-actions">
                                  <button className="docs-action-btn" onClick={() => handleViewSourceText(src.source, f.filename)}>VIEW</button>
                                  <button className="docs-action-btn docs-action-delete" onClick={() => handleDeleteDocument(src.source, f.filename)}>DEL</button>
                                </span>
                              </div>
                            ))}
                          </div>
            ...
            {/* Generated Data */}
            {!allDocsLoading && allDocSources?.sources && (() => {
              const generatedTypes = ['dma_sections', 'timeline', 'regulatory', 'tracking', 'overrides', 'termination', 'covenants', 'mae', 'reddit'];
              const generated = allDocSources.sources.filter((s: any) => generatedTypes.includes(s.source));
              return (
                <div className="fin-card" style={{ marginTop: 'var(--space-lg)' }}>
            ...
            {docViewSource && (
              <div className="fin-card" style={{ marginTop: 'var(--space-lg)' }}>
                <div className="fin-card-title" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  Source: {docViewSource.type}{docViewSource.filename ? ` / ${docViewSource.filename}` : ''}
                  <button className="pr-toggle-btn" onClick={() => { setDocViewSource(null); setDocSourceText(null); }}>CLOSE</button>
                </div>
                {docSourceLoading ? (
            ...
            {/* Cross-Source Discrepancies */}
            {allDocSources?.discrepancies && allDocSources.discrepancies.length > 0 && (
              <div className="fin-card" style={{ marginTop: 'var(--space-lg)' }}>
                <div className="fin-card-title" style={{ color: 'var(--accent-yellow)' }}>Cross-Source Discrepancies</div>
                <div className="docs-disc-subtitle">Fields where Press Release, DMA Summary, and Timeline disagree</div>
                <div className="docs-disc-fields">
                  <div className="docs-disc-checked">
                    Checked: Offer Price, Exchange Ratio, Deal Type, Outside Date, Expected Close, Announce Date
                  </div>
                </div>
                <div className="docs-inventory">
                  <div className="docs-header-row">
                    ...
                    <span className="docs-col-detail" style={{ flex: 2 }}>Value</span>
                    <span className="docs-col-actions"></span>
                  </div>
                  {allDocSources.discrepancies.map((d: any, i: number) => (
                    (d.sources || []).map((s: any, j: number) => (
                      <div key={`${i}-${j}`} className={`docs-row ${s.source === d.most_recent ? 'docs-row-newest' : ''}`}>
                        <span className="docs-col-type">{j === 0 ? (d.label || d.field) : ''}</span>
                        <span className="docs-col-status docs-disc-source">
                          {s.source === 'press_release' ? 'PR' :
                           s.source === 'dma_extract' ? 'DMA' :
                           s.source === 'timeline' ? 'Timeline' : s.source}
                        </span>
                        <span className="docs-col-detail" style={{ flex: 2 }}>
                          {String(s.value)}
                          {s.source === d.most_recent && <span className="docs-disc-newest">NEWEST</span>}
                        </span>
                        <span className="docs-col-actions"></span>
                      </div>
                    ))
